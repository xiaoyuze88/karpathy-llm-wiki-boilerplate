...
│   ├── log.md             # 操作时间线（只追加）
│   ├── hot.md             # 当前会话焦点缓存
│   ├── sources/           # 每份来源的摘要页（示例 3 页）
│   ├── concepts/          # 概念解释页（示例 3 页）
│   ├── entities/          # 人物 / 公司 / 产品页（示例 1 页）
│   ├── comparisons/
│   └── syntheses/
└── .claude/
    └── skills/            # 三种核心操作的执行手册
...
```
# 把文章放入 raw/articles/，然后告诉 Agent：
Ingest raw/articles/你的文章.md
```

Agent 会：读文件 → 提取要点 → 建摘要页 → 更新概念/实体页 → 更新 index/log → Git push。
...
Agent 会：扫描矛盾、孤立页、缺失概念、过时声明 → 输出诊断报告。
**建议每 20 次摄入或每月跑一次。**
